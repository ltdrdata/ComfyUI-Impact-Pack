...
class ImpactWildcardProcessor:
    ...
    def INPUT_TYPES(s):
        return {"required": {
            "wildcard_text": ("STRING", {"multiline": True, "dynamicPrompts": False, "tooltip": "Enter a prompt using wildcard syntax. Wildcards must be defined in ComfyUI-Impact-Pack/wildcards."}),
            "populated_text": ("STRING", {"multiline": True, "dynamicPrompts": False, "tooltip": "The actual value passed during the execution of 'ImpactWildcardProcessor' is what is shown here. Wildcards can also be used in 'populated_text'."}),
    ...
    DESCRIPTION = (
        "The 'ImpactWildcardProcessor' processes text prompts written in wildcard syntax and outputs the processed text prompt.\n\n"
        "TIP: Before the workflow is executed, the processing result of 'wildcard_text' is displayed in 'populated_text', and the populated text is saved along with the workflow. "
        "If you want to use a seed-converted input, write the prompt directly in 'populated_text' and set the mode to 'fixed'."
    )
    ...
    def process(text, seed=0):
        return impact.wildcards.process(text=text, seed=seed)

    def doit(self, *args, **kwargs):
        mode = kwargs.get("mode", "populate")
        seed = kwargs.get("seed", 0)
        wildcard_text = kwargs.get("wildcard_text", "")
        populated_text = kwargs.get("populated_text", "")

        if mode == "fixed":
            # Use the existing populated_text without processing
            result = populated_text
        else:
            # If there is wildcard_text, process it; otherwise fall back to populated_text
            if wildcard_text.strip():
                result = ImpactWildcardProcessor.process(text=wildcard_text, seed=seed)
            else:
                result = populated_text
